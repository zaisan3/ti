...
import com.zaisan.ti.web.pagecache.constructs.HttpDateFormatter;
...
public class SimpleCachingHeadersPageCachingFilter extends SimplePageCachingFilter {
     ...
     */
    @Override
    protected PageInfo buildPage(HttpServletRequest request, HttpServletResponse response, FilterChain chain,CacheStrategyCarrier cacheStrategyCarrier)
            throws AlreadyGzippedException, Exception {
        PageInfo pageInfo = super.buildPage(request, response, chain,cacheStrategyCarrier);

        final List<Header<? extends Serializable>> headers = pageInfo.getHeaders();

        long ttlMilliseconds = calculateTimeToLiveMilliseconds(pageInfo);
        ...
        headers.add(new Header<String>("Cache-Control", cacheStrategyCarrier.getCacheCtrolStr()));
     ...
     */
    protected long calculateTimeToLiveMilliseconds(PageInfo pageInfo) {
/*        if (blockingCache.isDisabled()) {
    	...
    	return pageInfo.getTimeToLiveSeconds() * MILLISECONDS_PER_SECOND;
