...
import com.zaisan.ti.web.pagecache.constructs.AlreadyCommittedException;
...
import com.zaisan.ti.web.pagecache.constructs.Header;
import com.zaisan.ti.web.pagecache.constructs.PageInfo;
import com.zaisan.ti.web.pagecache.constructs.ResponseHeadersNotModifiableException;
import com.zaisan.ti.web.pagecache.constructs.ResponseUtil;
import com.zaisan.ti.web.pagecache.constructs.SerializableCookie;
 ...
 * <p/>
 * The filters must be declared in the web.xml deployment descriptor. Then a mapping from a web resource, such as a JSP Page, FreeMarker page, Velocity page, Servlet or static resouce needs to be defined. Finally, a succession of mappings can be used to
 * create a filter chain. See SRV.6 of the Servlet 2.3 specification for more details.
 * <p/>
...
public abstract class CachingFilter extends Filter{
     ...
     * <p/>
     * A timeout can be appled using <code>blockingCache.setTimeoutMillis(int timeout)</code> and takes effect immediately for all new requests
     * 
     ...
     * <p/>
     * By default this method will queue requests requesting the page response for a given key until the first thread in the queue has completed. The request which occurs when the page expires incurs the cost of waiting for the downstream processing to
     * return the respone.
     * <p/>
     ...
     * @throws Exception
     *             for all other exceptions. They will be caught and logged in {@link Filter#doFilter(javax.servlet.ServletRequest, javax.servlet.ServletResponse, javax.servlet.FilterChain)}
     */
    protected void doFilter(final HttpServletRequest request,final HttpServletResponse response,final FilterChain chain) throws AlreadyGzippedException,AlreadyCommittedException,FilterNonReentrantException,LockTimeoutException,Exception{
        ...
        if(pageInfo==null){ // cache代理服务 不可用 （原因之一：  获取 cache的读锁超时，则绕过 cache代理）
        	chain.doFilter(request, response);
            return;
        }
        if (pageInfo.isOk()){
     ...
     */
    protected PageInfo buildPageInfo(final HttpServletRequest request,final HttpServletResponse response,final FilterChain chain,CacheStrategyCarrier cacheKeyCarrier) throws Exception{
        // Look up the cached page
        PageInfo pageInfo = null;
        String cacheKey = cacheKeyCarrier.getCacheKey();
        /*Integer timeToIdleSeconds = cacheKeyCarrier.getTimeToIdleSeconds();*/
        Integer timeToLiveSeconds = cacheKeyCarrier.getTimeToLiveSeconds();
        try{
            checkNoReentry(request);
            Element element = blockingCache.get(cacheKey);
            if (element == null || element.getObjectValue() == null){
                try{
                    // Page is not cached - build the response, cache it, and
                    // send to client
                    pageInfo = buildPage(request, response, chain,cacheKeyCarrier);
                    if (pageInfo.isOk()){
                        if (LOG.isDebugEnabled()){
                            LOG.debug("PageInfo ok. Adding to cache " + blockingCache.getName() + " with key " + cacheKey);
                        }
                        blockingCache.put(new Element(cacheKey, pageInfo, false, null, timeToLiveSeconds));
                    }else{
                        if (LOG.isDebugEnabled()){
                            LOG.debug("PageInfo was not ok(200). Putting null into cache " + blockingCache.getName() + " with key " + cacheKey);
                        }
                        blockingCache.put(new Element(cacheKey, null,false, null, timeToLiveSeconds));
                    }
            ...
            /*sang modify// do not release the lock, because you never acquired it
            throw e;*/
        	// 若 前期 读操作 超时，直接返回 pageInfo=null, 后续将直接 透传 进后面的 springServlet
        	// 若 后期 写操作 超时， 毕竟本次内容已经拿到，直接返回算了，总之 不能 把 LockTimeoutException 抛出去
        	return pageInfo;  
        }finally{
     ...
     * @throws Exception
     */
    protected PageInfo buildPage(final HttpServletRequest request,final HttpServletResponse response,final FilterChain chain,CacheStrategyCarrier cacheStrategyCarrier) throws AlreadyGzippedException,Exception{
        ...
        wrapper.flush();
        Integer timeToLiveSeconds = cacheStrategyCarrier.getTimeToLiveSeconds();
        if(timeToLiveSeconds==null){
        	timeToLiveSeconds = (int) blockingCache.getCacheConfiguration().getTimeToLiveSeconds();
        }
     ...
     * <p/>
     * The <code>cacheName</code> field is be set by the <code>doInit</code> method. Override to control the name used. The significance is that the name is used to find a cache configuration in <code>ehcache.xml</code>
     * 
     ...
     * <li>Whether additional parameters used for referral tracking e.g. google should be excluded to maximise cache hits
     * <li>Additional parameters can be added to any page. The page will still work but will miss the cache. Consider coding defensively around this issue.
     * </ul>
     * <p/>
     ...
     */
    protected abstract CacheStrategyCarrier calculateKey(final HttpServletRequest httpRequest);
    ...
    protected WebApplicationContext findWebApplicationContext() {
        ...
        if (attrName != null) {
            return WebApplicationContextUtils.getWebApplicationContext(this.getFilterConfig().getServletContext(), attrName);
        } else {
            return WebApplicationContextUtils.getWebApplicationContext(this.getFilterConfig().getServletContext());
        }
    }
